  ...
  return (
    <>
      ...
      <Main>
        ...
        <Box>
          {selectedId ? <MovieDetails selectedId={selectedId} onClose={clearSelect} /> : <>
    ...
    {loading ? <Loader /> : <>
      ...
      <section>
        <div className="rating">
          <StarRating starLength={10} size={24} />
        </div>
        <p><em>{plot}</em></p>
        <p>Starring {actors}</p>
...
function WatchedList({ watched }) {
  return (
    <ul className="list">
      {watched.map((movie) => <Watched key={movie.imdbID} movie={movie} />)}
    </ul>
  );
}
